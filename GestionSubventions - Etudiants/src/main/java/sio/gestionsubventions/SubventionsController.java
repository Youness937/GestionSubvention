...
//        lesSubventions.put("Paris",lesSecteursDeParis);
    ...
    public void btnAffecterSubventionClicked(Event event)
    {
        ...
        else
        {
            ...
            Structure laVille = new Structure(lvVilles.getSelectionModel().getSelectedItem().toString());
            if(!lesSubventions.containsKey(lvVilles.getSelectionModel().getSelectedItem().toString()))
            {
                ArrayList<Structure> lesVilles = new ArrayList<>();
                lesVilles.add(laVille);
            }
            Structure sect = new Structure(lvSecteurs.getSelectionModel().getSelectedItem().toString());
            if (!lesSubventions.containsKey(lvSecteurs.getSelectionModel().getSelectedItem().toString())){
                ArrayList<Structure> lesStructures = new ArrayList<>();
                lesStructures.add(sect);
            }
        }
    ...
    public void lvVillesStatsClicked(Event event)
    {
        ...
        TreeItem ville = null;
        TreeItem secteur = null;
        String villeChoisit = lvVilles.getSelectionModel().getSelectedItem().toString();
        String nomSecteur = lvSecteurs.getSelectionModel().getSelectedItem().toString();
        String montantSecteur = txtMontant.getText();
        if (lesSubventions.containsKey(lvVilles))
        {
            ville = new TreeItem(villeChoisit);
        }
        if (lesSubventions.containsKey(lvSecteurs))
        {
            secteur = new TreeItem(nomSecteur + " : " + montantSecteur);
        }
